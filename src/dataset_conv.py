        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in tqdm(lines):
                ...
                data = {
                    ...
                    'conv_id':      str(dialog.get('conv_id', 'unknown')),
...
class CRSConvDataCollator:
    ...
    def __call__(self, data_batch):
        ...
        # ── [FIUP] 透传画像字段（不做 padding，保持原始列表）────────────────
        input_batch['user_id']      = [d.get('user_id',      'unknown') for d in data_batch]
        input_batch['conv_id']      = [d.get('conv_id',      'unknown') for d in data_batch]
        input_batch['context_str']  = [d.get('context_str',  '')        for d in data_batch]
        input_batch['entity_names'] = [d.get('entity_names', [])        for d in data_batch]
        # ────────────────────────────────────────────────────────────────────
